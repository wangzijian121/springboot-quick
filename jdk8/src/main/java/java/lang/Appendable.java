...
/**
 * 可以向其追加字符序列和值的对象。Appendable接口必须由任何类实现，这些类的实例旨在接收来自java.util.Formatter的格式化输出。
 * 要追加的字符应该是有效的Unicode字符，如Unicode字符表示中所述。注意，补充字符可能由多个16位字符值组成。
 * 可附件对于多线程访问并不一定是安全的。线程安全是扩展和实现这个接口的类的责任。
 * 由于此接口可能由具有不同错误处理风格的现有类实现，因此不能保证错误将传播给调用者。
 */
